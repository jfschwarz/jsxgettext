...
function genBabel (babelSources, options) {
  Object.keys(babelSources).forEach(function(filename) {
    try {
      babelSources[filename] = parseBabel(babelSources[filename], options);  
    } catch(e) {
      console.error("Error parsing file: " + filename, e)
    }
  });

  return gen(babelSources, options);
}
...
function parseBabel(str, options) {
  return babel.transform(str, options.babelOptions).code;
};
...
exports.generateFromBabel      = genBabel;
