...
function checkExprNgettext(node, keyword) {
  var firstArg = node.arguments && node.arguments[0];
  var secondArg = node.arguments && node.arguments[1];
  return (node.type === "CallExpression" &&   // must be a call expression
            (
              node.callee.name &&  // Should not be an anonymous function call
              (node.callee.name === 'ngettext' ||            // with a gettext call expr
              (node.callee.name === keyword)) ||            // or keyword call expr
              (node.callee.type === 'MemberExpression' &&   // or a member expr
    ...
    firstArg && (isStrConcatExpr(firstArg) || isStringLiteral(firstArg)) &&
    secondArg && (isStrConcatExpr(secondArg) || isStringLiteral(secondArg))
  );
}
  ...
  Object.keys(sources).forEach(function (filename) {
    ...
    traverse(ast, {
      ...
      enter: function (node) {
        ...
        var isNgettextExpr = checkExprNgettext(node, options.plural);

        if (!isNgettextExpr && !checkExpr(node, options.keyword))
          return;
        ...
        if (isNgettextExpr)
          var strPlural = extractStr(node.arguments[1]);
        ...
        if (!translations[str]) {
          ...
          if (isNgettextExpr) {
            translations[str].msgid_plural = strPlural;
            translations[str].msgstr.push("");
          }
        } else {
          ...
          if (isNgettextExpr) {
            ...
            if (translations[str].msgid_plural)
              delete translations[str].msgid_plural;
            translations[str].msgstr = [];
          }
        }
      }
...
function genJSX (jsxSources, options) {
  Object.keys(jsxSources).forEach(function(filename) {
    jsxSources[filename] = parseJSX(jsxSources[filename], filename);
  });

  return gen(jsxSources, options);
}
...
function parseJSX(str, filename) {
  str = str.trim();

  if(str.indexOf("/** @jsx React.DOM */") !== 0) {
    throw new Error(filename + " does not start with jsx transformer hint /** @jsx React.DOM */");
  }

  return react.transform(str);
};
...
exports.generateFromJSX        = genJSX;
